package main

import (
	"github.com/xyproto/vt"
	"os"
	"sync"
	"time"
)

type Bob struct {
	color string
	x, y  int
	state rune
}

func (b *Bob) ToggleColor() {
	const c1 = "Red"
	const c2 = "Yellow"
	if b.color == c1 {
		b.color = c2
	} else {
		b.color = c1
	}
}

func (b *Bob) ToggleState() {
	const up = 'O'
	const down = 'o'
	if b.state == up {
		b.state = down
	} else {
		b.state = up
	}
}

func (b *Bob) Draw(c *vt.Canvas) {
	c.PlotColor(uint(b.x), uint(b.y), vt.LightColorMap[b.color], b.state)
}

func (b *Bob) Right(c *vt.Canvas) bool {
	b.x += 1
	if b.x >= int(c.W()) {
		b.x -= 1
		return false
	}
	return true
}

func (b *Bob) Left(c *vt.Canvas) bool {
	if b.x-1 < 0 {
		return false
	}
	b.x -= 1
	return true
}

func (b *Bob) Up(c *vt.Canvas) bool {
	if b.y-1 < 0 {
		return false
	}
	b.y -= 1
	return true
}

func (b *Bob) Down(c *vt.Canvas) bool {
	b.y += 1
	if b.y >= int(c.H()) {
		b.y -= 1
		return false
	}
	return true
}

func main() {

	c := vt.NewCanvas()
	tty, err := vt.NewTTY()
	if err != nil {
		panic(err)
	}

	var bob Bob
	bob.state = 'o'
	bob.color = "Yellow"
	bob.x = 10
	bob.y = 10

	var draw sync.RWMutex
	sigChan := make(chan os.Signal, 1)
	vt.SetupResizeHandler(sigChan)
	go func() {
		for range sigChan {
			// Terminal was resized

			// Prepare to resize the canvas
			draw.Lock()

			// Clear the screen after the resize
			vt.Clear()

			// Create a new canvas, with the new size
			nc := c.Resized()
			if nc != nil {
				c.Clear()
				c.Draw()
				c = nc
			}
			// Clear again, for good measure
			vt.Clear()
			// Redraw the characters
			c.Redraw()
			// Done
			draw.Unlock()
		}
	}()

	vt.Clear()
	vt.ShowCursor(false)
	vt.SetLineWrap(true)

	running := true
	for running {

		//vt.Clear()

		// Draw elements in their new positions
		draw.Lock()
		bob.Draw(c)
		draw.Unlock()

		// Update the canvas
		draw.Lock()
		c.Draw()
		draw.Unlock()

		// Wait a bit
		time.Sleep(time.Millisecond * 10)

		// Change state
		oldx := bob.x
		oldy := bob.y
		moved := false

		// Handle events
		draw.Lock()
		switch tty.Key() {
		case vt.KeyArrowUp:
			moved = bob.Up(c)
		case vt.KeyArrowDown:
			moved = bob.Down(c)
		case vt.KeyArrowRight:
			moved = bob.Right(c)
		case vt.KeyArrowLeft:
			moved = bob.Left(c)
		case vt.KeyEsc, 'q':
			running = false
			break
		case vt.KeySpace:
			bob.ToggleColor()
		}
		draw.Unlock()

		if moved {
			bob.ToggleState()

			// Erase elements at their old positions
			draw.Lock()
			c.Plot(uint(oldx), uint(oldy), ' ')
			draw.Unlock()
		}
	}
	tty.Close()

	vt.SetLineWrap(true)
	vt.ShowCursor(true)
	vt.Home()
}
